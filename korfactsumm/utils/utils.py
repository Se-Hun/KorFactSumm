...
@dataclass
class Config:
    NER_MODEL: str = "monologg/koelectra-base-v3-naver-ner"
    REL_MODEL: str = ""
    QG_MODEL: str = ""
    QA_MODEL: str = "monologg/koelectra-base-v3-finetuned-korquad"
    SUMM_MODEL: str = "gogamza/kobart-summarization"
...
def grouped_entities(entities: List[Dict]) -> List:
    ...
    def _remove_prefix(entity: str) -> str:
        if "-" in entity:
            entity_string = entity.split("-")
            if entity_string[0] > entity_string[1]:
                entity = entity_string[0]
            else:
                entity = entity_string[1]
        return entity
    ...
    for pair in entities[1:]:
        ...
        if entity == prev_entity:
            if entity == "O":
                ...
                prev_start = start
                prev_end = end
            if "I-" in entity or "-I" in entity:
                prev_word += f" {word}"
                prev_end = end
        elif (entity != prev_entity) and (("I-" in entity) or ("-I" in entity)) and (type != "O"):
            prev_word += f" {word}"
            prev_end = end
        else:
            _append(result, prev_word, prev_type, prev_start, prev_end)
